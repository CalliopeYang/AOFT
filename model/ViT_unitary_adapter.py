...
class UnitaryLinear_adapter(nn.Module):
    def __init__(self, config, bias=True, enable_unitary=False):
        super(UnitaryLinear_adapter, self).__init__()
        self.enable_unitary = enable_unitary
        self.size_in = config.hidden_size
        self.size_out = config.hidden_size
        # self.mlp = nn.Linear(size_in, size_out, bias=bias)

        self.q_left = nn.Parameter(torch.zeros(self.size_in - 1, 1, dtype=torch.float32))

        self.q_right = nn.Parameter(torch.zeros(self.size_out - 1, 1, dtype=torch.float32))
        ...
        self.shape = 8
        # self.q_lambda = nn.Parameter(torch.zeros(self.shape, self.shape))
        self.q_lambda = nn.Parameter(torch.rand(1, self.shape, dtype=torch.float32))
    ...
    def forward(self, x):
        ...
        weight = (left_unitary * self.q_lambda @ right_unitary.T).T

        # TODO 加偏置，加relu   # 加relu不行
...
class Lora_Linear(nn.Module):
    def __init__(self, size_in, size_out, bias=True, enable_lora=False):
...
class Attention(nn.Module):
    def __init__(self, config, vis):
        ...
        self.query = Lora_Linear(config.hidden_size, self.all_head_size, bias=True)
        self.key = Lora_Linear(config.hidden_size, self.all_head_size, bias=True)
        self.value = Lora_Linear(config.hidden_size, self.all_head_size, bias=True)
        self.out = Lora_Linear(config.hidden_size, config.hidden_size, bias=True)
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = Lora_Linear(config.hidden_size, config.transformer["mlp_dim"], bias=True)
        self.fc2 = Lora_Linear(config.transformer["mlp_dim"], config.hidden_size, bias=True)
...
class Block(nn.Module):
    def __init__(self, config, vis, drop_path=0.0, enable_lora=False):
        ...
        self.adapter1 = UnitaryLinear_adapter(config)             # attention
        self.adapter2 = UnitaryLinear_adapter(config)               #ffn
        self.adapter1_norm = nn.LayerNorm(config.hidden_size, eps=1e-6)    # attention        
        self.adapter2_norm = nn.LayerNorm(config.hidden_size, eps=1e-6)        #ffn


    def forward(self, x):
        x = x + self.drop_path1(self.attn(self.attention_norm(x)))
        x = x + self.adapter1(self.adapter1_norm(x))               # attention
        x = x + self.drop_path2(self.ffn(self.ffn_norm(x)))
        x = x + self.adapter2(self.adapter2_norm(x))                  # ffn
        return x
